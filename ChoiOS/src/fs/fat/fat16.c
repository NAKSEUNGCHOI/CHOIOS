...
#define CHOIOS_FAT16_SIGNATURE          0x29
#define CHOIOS_FAT16_FAT_ENTRY_SIZE     0x02
#define CHOIOS_FAT16_BAD_SECTOR         0xFF7
#define CHOIOS_FAT16_UNUSED             0x00
...
// infomation in fat header file is suficient to read the fat16 file system most of the time
// sometimes you need additional data from fat header file to read it.
struct fat_h
{
...
// fat directory entry is used to find out if it is a file or subdirectory.
struct fat_directory_item
{
    uint8_t filename[8];
    uint8_t ext[3];
    uint8_t attribute; // definitions declared above are going here.
    uint8_t reserved;
    uint8_t creation_time_tenths_of_a_sec;
    uint16_t creation_time;
    uint16_t creation_date;
    uint16_t last_access;
    // if this is a file, the high 16 bots of where the first data for this file is stored.
    // if this is a subdirectory, these are the high 16 bits of where the subdirectories are stored.
...
// internal structure that makes it easier to calculate things such as uint16_t for high and low bits of clusters
struct fat_directory
{
    struct fat_directory_item *item;
    int total; // total no of items
    int sector_pos;
...
// declaring an instance of the struct filesystem and initializing its member.
...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector)
{
    ...
    if(diskstreamer_seek(stream, directory_start_pos) != CHOIOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    while(1)
    {
        if (diskstreamer_read(stream, &item, sizeof(item)) != CHOIOS_ALL_OK)
        {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory)
{
    ...
    struct disk_stream* stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != CHOIOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != CHOIOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
{
    int res = 0;
    // by using fat_private, we're caching the data to be accessed and used.
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != CHOIOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    
    // if fat16 signature is not present, meaning that the header we loaded above at line 164
    // does not have a valid fat16 signature, we're not responsible for handling whatever
    // this filesystem is on this disk.
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != CHOIOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
{
    // try to find a null terminator or space
    // e.g. file with 5 space behind.
    // we only extract file without the 5 space at the end.
...
static int fat16_get_fat_entry(struct disk* disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * CHOIOS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++)
    {
        ...
        if (entry == CHOIOS_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
...
struct fat_directory* fat16_load_fat_directory(struct disk* disk, struct fat_directory_item* item)
{
    ...
    struct fat_private* fat_private = disk->fs_private;
    // we're trying to load a directory here. this filters out files passed to us.
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name)
{
    ...
    char tmp_filename[CHOIOS_MAX_PATH];
    for (int i = 0; i < directory->total; i++)
    {
        // get us the name of the file and the extension of the file .e.g. text and .txt
...
struct fat_item* fat16_get_directory_entry(struct disk* disk, struct path_part* path)
{
    ...
    // e.g. 0:/abc/text.txt
    // this line below looks for the next item in the directory.
    // if the next path is a directory, it'll return fat item associated with directory.
    // On the other hand, if it was a file, it returns a fat item associated with the file.
